...
var ImageViewer = /** @class */ (function (_super) {
    ...
    function ImageViewer() {
        ...
        // 缩放大小
        _this.scale = INITIAL_SCALE;
        _this.animatedScale = new react_native_1.Animated.Value(INITIAL_SCALE);
        ...
        _this.resetScale = function () {
            ...
            _this.positionY = 0;
            _this.scale = INITIAL_SCALE;
            _this.animatedScale.setValue(INITIAL_SCALE);
        };
        ...
        this.imagePanResponder = react_native_1.PanResponder.create({
            ...
            onPanResponderGrant: function (evt) {
                ...
                if (evt.nativeEvent.changedTouches.length > 1) {
                    var centerX = (evt.nativeEvent.changedTouches[0].locationX + evt.nativeEvent.changedTouches[1].locationX) / 2;
                    _this.centerDiffX = centerX - _this.props.cropWidth / 2;
                    var centerY = (evt.nativeEvent.changedTouches[0].locationY + evt.nativeEvent.changedTouches[1].locationY) / 2;
                ...
                if (evt.nativeEvent.changedTouches.length <= 1) {
                    ...
                    if (new Date().getTime() - _this.lastClickTime < (_this.props.doubleClickInterval || 0)) {
                        ...
                        _this.doubleClickX = evt.nativeEvent.changedTouches[0].locationX;
                        _this.doubleClickY = evt.nativeEvent.changedTouches[0].locationY;
                        // 缩放
                        _this.isDoubleClick = true;
                        if (_this.props.enableDoubleClickZoom) {
                            if (_this.scale > INITIAL_SCALE || _this.scale < INITIAL_SCALE) {
                                // 回归原位
                                _this.scale = INITIAL_SCALE;
                                _this.positionX = 0;
                                _this.positionY = 0;
                                ...
                                // 开始缩放
                                var maxScale = _this.props.maxScale || 2;
                                _this.scale = Math.min(maxScale, 2);
                                // 缩放 diff
    ...
    ImageViewer.prototype.getImagePosition = function () {
        return {
            positionX: this.positionX,
            positionY: this.positionY,
            scale: this.scale,
